...
const whoFor = [
  {
    icon: Rocket,
    title: "Freelancers & Founders",
    desc: "Separate business income from personal spending from day one.",
  },
  {
    icon: Users,
    title: "Growing Teams",
    desc: "Give trusted team members visibility without handing over full account access.",
  },
  {
    icon: Building2,
    title: "Established Companies",
    desc: "Move larger volumes with the oversight a real operation needs.",
  },
];
...
const features = [
  {
    icon: FileText,
    title: "Invoicing & Vendor Payments",
    desc: "Transfers built around how businesses actually pay — clear references, clear records.",
  },
  {
    icon: Users,
    title: "Multi-User Oversight",
    desc: "Keep an eye on team spending without sharing a single login.",
  },
  {
    icon: Tags,
    title: "Transaction Categorization",
    desc: "Every transfer tagged automatically, so your books stay clean.",
  },
  {
    icon: ShieldCheck,
    title: "Priority Manual Review",
    desc: "Business applications get a dedicated look from our approval team.",
  },
];

const requirements = [
  "Business name and registered address",
  "A valid government-issued ID for the account holder",
  ...
  <div className="bg-slate-50 min-h-screen">
    ...
    <section className="max-w-7xl mx-auto px-4 md:px-8 pt-16 md:pt-20 grid md:grid-cols-2 gap-10 items-center">
      <div>
        ...
        <p className="text-slate-500 mt-5 text-base md:text-lg leading-relaxed">
          Banking that keeps up with your company, not just your personal
          spending — built for founders, freelancers, and growing teams who need
          more than a personal checking account can offer.
        </p>
        <Link
          to="/register"
      ...
      <div className="rounded-xl3 overflow-hidden shadow-card h-[280px] md:h-[360px]">
        <img
          src="/business1.jpg"
          alt="Well Trust Bank business account"
    ...
    <section className="max-w-7xl mx-auto px-4 md:px-8 py-16 md:py-20 grid md:grid-cols-5 gap-10 md:gap-14 items-center">
      <div className="md:col-span-2 rounded-xl3 overflow-hidden shadow-card h-[300px] md:h-[420px] order-2 md:order-1">
        <img
          src="/check.jpg"
          alt="Team reviewing business account activity"
          className="w-full h-full object-cover"
        />
      </div>
      <div className="md:col-span-3 order-1 md:order-2">
        ...
        <h2 className="text-3xl md:text-4xl font-bold text-navy-900 mt-2 mb-6">
          Everything a business account needs
        </h2>
    ...
    <section className="max-w-3xl mx-auto px-4 md:px-8 py-10 md:py-14">
      <div className="card p-6 md:p-8">
        <h3 className="font-bold text-navy-900 text-lg mb-4 flex items-center gap-2">
          <TrendingUp size={19} className="text-gold-600" /> What you'll need to
          apply
        </h3>
        ...
        <p className="text-xs text-slate-400 mt-5">
          Business applications go through the same manual review process as
          every Well Trust Bank account — most are approved within 1-2 business
          days.
        </p>
      </div>
    </section>
    ...
    <section className="max-w-7xl mx-auto px-4 md:px-8 pb-24">
      <div className="rounded-xl3 bg-gradient-to-br from-navy-800 to-navy-900 px-8 md:px-16 py-14 text-center relative overflow-hidden">
        ...
        <h2 className="text-3xl md:text-4xl font-bold text-white relative">
          Ready to bank for your business?
        </h2>
        <p className="text-slate-300 mt-3 max-w-lg mx-auto relative">
          Opening a business account takes about five minutes.
        </p>
        <Link
          to="/register"
          className="btn-gold inline-flex items-center gap-2 mt-8 relative"
        >
          Open a Business Account <ArrowRight size={17} />
        </Link>
      </div>
    </section>
